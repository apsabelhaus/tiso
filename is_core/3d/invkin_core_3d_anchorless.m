%% invkin_core_3d_anchorless.m
% Copyright Andrew P. Sabelhaus 2019

function [f_opt, q_opt, A, p, lengths] = invkin_core_3d_anchorless(x, y, z, px, py, pz, C, s, q_min, anchors_vec, debugging)
...
%
% In addition, this 'anchorless' function takes in an additional vector
% that removes certain nodes from the calculations - ones where the
% external reaction forces don't matter or are neglected.
%
% Inputs:
...
%
%   anchors_vec = \in {0, 1}^n, an n-dimensional vector. = 1 if node is to
%       be 'kept' as part of the computation, or = 0 if the node is an
%       anchor and is to be discarded. No force balance will be calulcated
%       at '0' nodes, and those elements of px,py,pz will be discarded.
%
...
%% Startup message
if debugging >= 1
    disp('Starting invkin_core_3d_anchorless...');
end
...
% Calculate the anchor-removal matrix.
% For using the anchors-formulation (removing the anchors from the
% constraint), we left-multiply the A matrix.
% Each dimensional component needs to be
G = diag(anchors_vec);
% Remove the zero-ed rows in G. It doesn't seem like there's a nice linear
% algebra way to do this, so use MATLAB's logical indexing instead.
G(~any(G,2), :) = [];

% This G takes out the anchors for one dimension.
...
% is
% NOTE: made into a column vector here.
...
% Then, elementwise calculate the optimal forces.
% Both need to be vectors of the same dimension and shape.
f_opt = q_opt .* lengths;
