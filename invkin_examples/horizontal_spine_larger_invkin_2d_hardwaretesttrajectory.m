...
% (playing with some ratios here to see what happens)
translation_0 = [calibrated_position_free_x - width/3;
                 calibrated_position_free_y];
...
rot_axis_pt = [ (C_x + A_x); C_y];


%we don't want it to sweep out from the tip of the
...
for i=1:num_points
    ...
    % vertebra.
    x(1:3, i) = coordinates_i_fixed(1,:)';
    x(4:7, i) = coordinates_i_free(1,:)';
    y(1:3, i) = coordinates_i_fixed(2,:)';
    y(4:7, i) = coordinates_i_free(2,:)';
%     x(:,i) = coordinates_i(1,:)';
%     y(:,i) = coordinates_i(2,:)';
end
...
% ^2019-05-13: We are ignoring the force/moment balance for the free
% verterba, so no longer need to worry aobut the external reaction forces
% at all, actually.
...
savefile_path = strcat(pwd, filesep);
